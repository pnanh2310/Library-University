...
// Đếm tổng
$count_sql = "SELECT COUNT(*) AS total FROM sach 
              WHERE ten_sach LIKE ? OR tac_gia LIKE ? OR the_loai LIKE ?";
...
if ($total_records == 0) {
    echo "<div class='alert alert-warning text-center'>Không tìm thấy kết quả cho <b>$keyword</b>.</div>";
    exit;
}

echo "<div class='row mt-3 g-4'>";
while ($row = $result->fetch_assoc()) {
    $url = "../view/muon_sach.php?id={$row['id']}&ten=" . urlencode($row['ten_sach']);
